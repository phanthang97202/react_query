...
export default function AddStudent() {
  ...
  // tham chiếu đến  QueryClient ở file tổng index.js
  const queryClient = useQueryClient()

  // lấy ra thông tin người dùng để chỉnh sửa
  const { id } = useParams()
  useQuery({
  ...
  const updateStudentMutation = useMutation({
    ...
    // cập nhật lại danh sách sinh viên khi update
    onSuccess: (data) => {
      // lấy ra tên của thằng muốn update để set lại chính cái mà sau khi đã update
      queryClient.setQueryData(['getStudent', id], data)
    }
  })
  ...
  console.log('===error', addStudentMutation.error)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    // cách 2 thì chúng ta dùng mutateAsync trẳ về promise
    if (isAddMode) {
      try {
    ...
    //   onSuccess: () => {
    //     // đặt đây thì chỉ khi nào thành công thì mới set lại form ban đầu
    //     setFormState(initialFormState)
    //   }
    // })
    // đặt đây thì kể cả đúng hay sai thì nó vẫn reset lại form
    // setFormState(initialFormState)
  }
  ...
  return (
    <div>
      ...
      {addStudentMutation.isLoading ? <p className='text-black-500'>WAITING...</p> : <></>}
